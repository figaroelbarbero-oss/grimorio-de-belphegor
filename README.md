# ⛧ EL GRIMORIO DE BELPHEGOR ⛧

### No deberías estar leyendo esto.

Si estás aquí, ya es demasiado tarde. El enlace que seguiste no fue un accidente. Fue una invitación. Y las invitaciones de Belphegor no se rechazan.

---

**El Grimorio de Belphegor** es un juego de horror interactivo que no se queda dentro de tu pantalla.

Escucha tu micrófono. Sabe tu nombre. Recuerda cómo moriste la última vez. Y cuando cambies de pestaña para escapar... se dará cuenta.

No es un juego. Es un ritual disfrazado de página web.

---

## Lo que te espera

- 🩸 **66 escenas** de terror narrativo con decisiones que destruyen tu alma
- 👁️ **Un ojo que te sigue** — literalmente sigue tu cursor
- 🎤 **Escucha tu habitación** — reacciona a tu silencio y a tus gritos
- 📛 **Usa tu nombre** contra ti en susurros personalizados
- ⛧ **Combate ritual** con pentagramas giratorios
- 🔮 **Hechizos trazables** — dibuja patrones con el mouse para lanzar magia
- 🪞 **3 perspectivas**: Juega como el humano, como el demonio, o como la guardiana eterna
- 💀 **12 finales** — incluyendo uno que no es realmente un final
- 🩸 **Ecos de sangre** — el juego recuerda a todos los que jugaron antes que tú
- 🔋 **Sabe cuánta batería te queda** — y te lo recuerda
- 📑 **La pestaña del navegador sabe** que intentaste huir

## Advertencia

> *Al hacer clic en "Abrir el Grimorio", aceptas que lo que ocurra después es consecuencia de tu propia curiosidad. Este juego contiene jumpscares, horror psicológico, manipulación del navegador, y contenido perturbador. No recomendado para menores de 16 años ni para personas sensibles a contenido de terror extremo.*

> *Belphegor no es responsable de pesadillas, parálisis de sueño, ni de la sensación de que alguien te observa después de jugar.*

---

## Jugar

**[⛧ ABRIR EL GRIMORIO ⛧](https://figaroelbarbero-oss.github.io/grimorio-de-belphegor/Helel%20Raise/grimorio_de_belphegor.html)**

Requiere navegador moderno con JavaScript habilitado. Auriculares recomendados. Luces apagadas. Solo.

### Invocarlo en tu propia máquina

```bash
git clone https://github.com/figaroelbarbero-oss/grimorio-de-belphegor.git
```

- **Versión principal**: abre `Helel Raise/grimorio_de_belphegor.html` directamente en el navegador (funciona desde `file://`).
- **Versión modular** (experimental): necesita un servidor estático —
  `cd "Helel Raise" && python3 -m http.server 8667` y visita `http://localhost:8667/`.

---

## Créditos

Desarrollado por **HELEL RAISE**

*Qui intrat, non revertetur.*
*(El que entra, no regresa.)*

---

<p align="center">⛧</p>
